...
class TestMotion(unittest.TestCase):
    ...
    def test_motion_points(self):
        motion = Motion(v_init = 10, v_final = 30, 
                accel = 10, decel = 5)
        d, t = motion.calc_accel_distance_time(v_max = 100)
        self.assertEqual(d, 495)
        self.assertEqual(t, 9)
        d, t = motion.calc_decel_distance_time(v_max = 100)
        self.assertEqual(d, 910)
        self.assertEqual(t, 14)
        motion_points = motion.calc_motion_points(distance = 1605, v_max = 100)
    ...
    def test_motion_max_distance(self):
        ...
        distance = motion.calc_max_distance(time = 25)
        velocity = motion.calc_velocity(time = 25, distance = distance)
        self.assertNear(distance, 1611.5, 0.5)
        self.assertNear(velocity, 106.5, 0.5)
